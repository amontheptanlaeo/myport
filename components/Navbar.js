...
function Navbar({mySkill , myAbout , myContact , myHome}) {
  ...
  return (
    <div className={`w-full bg-none ${isOpen ? 'h-screen items-start pt-[1.55rem] sticky':'h-20 items-center'} flex justify-between px-14  font-bold text-white text-[20px] absolute top-0 z-[9999] overflow-hidden `}>
        ...
        <div className={`absolute ${isOpen ? 'right-0 fixed':'right-[-1000px]'} sm:hidden w-full top-0 bg-[#E4778C] border-l-8 border-white h-screen flex justify-center items-center flex-col gap-10 transition-all duration-300 ease-linear`}>
        ...
        <motion.ul variants={container}
            ...
            <motion.li variants={item} className='cursor-pointer p-3 hover:border-b-2 transition-all duration-100 ease-linear hover:py-2 hover:text-[#A39EEF] hover:border-[#A39EEF] text-[16px] md:text-[20px]'onClick={() => myContact.current.scrollIntoView()}>Github</motion.li>
        </motion.ul>
    </div>
  )
}
